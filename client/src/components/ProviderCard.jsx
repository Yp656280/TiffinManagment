...
function ProviderCard({ providerLogo, name, address, rating, id }) {
  ...
  return (
    <>
      <Link
        to={`/provider/${id}`}
        className="flex flex-col p-2 border border-gray-300  hover:shadow-lg rounded-lg transition duration-300 ease-in gap-3"
      >
        ...
        <div className="flex flex-col gap-1 p-2 text-slate-900 font-sans">
          <div className="text-lg font-semibold flex justify-between items-center">
            <h1>{name}</h1>
            <span className="flex items-center bg-slate-800 rounded-lg px-1 text-white">
              <small>{`${rating === 0 ? "New" : rating}`}</small>
              <AiFillStar fontSize={"medium"} />
            </span>
          </div>
