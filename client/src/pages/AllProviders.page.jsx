...
function AllProvidersPage() {
  ...
  const handleSearch = () => {
    ...
    if (searchData.length > 0 && allProviders && allProviders.length > 0) {
      updatedSearchedProviders = allProviders.filter(
        (provider) =>
          provider.address.toLowerCase().search(searchData.toLowerCase()) !== -1
      );
    }
      ...
      {providers && (
        <div>
            ...
            <div className="flex items-center border rounded-full w-96 mx-3 shadow">
              <div className="flex items-center border bg-white w-full rounded-full overflow-hidden">
                <input
                  ...
                  className="w-full h-full px-4 py-2 border-l focus:outline-none flex items-center"
                  id="email"
                />
                <button
                  onClick={handleSearch}
                  className="flex items-center bg-slate-800 text-white h-10 px-3"
                >
                  <BiSearch />
                </button>
              </div>
          ...
          {isSearch &&
            searchData.length > 0 &&
            searchedProviders.length === 0 && (
              <p className="font-semibold text-gray-500 text-center w-full py-2.5">
                No Providers Found
              </p>
            )}
        </div>
